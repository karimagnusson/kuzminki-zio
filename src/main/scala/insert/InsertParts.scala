...
case class InsertParts[M <: Model, P](
  model: M,
  paramShape: ParamShape[P]
) {

  def toValuesParts(params: P) = ValuesParts(
    model,
    paramShape.cols,
    paramShape.conv.fromShape(params)
  )

  def toColl = SectionCollector(Vector(
    InsertIntoSec(ModelTable(model)),
    InsertColumnsSec(paramShape.cols),
  ))

  def toBlankColl = SectionCollector(Vector(
    InsertIntoSec(ModelTable(model)),
    InsertColumnsSec(paramShape.cols),
...
case class ValuesParts[M <: Model](
  model: M,
  ...
  def toWhereNotExistsColl(uniqueCols: Seq[TypeCol[_]]) = {
    ...
    SectionCollector(Vector(
      InsertIntoSec(ModelTable(model)),
      InsertColumnsSec(cols),
      InsertWhereNotExistsSec(values, ModelTable(model), WhereSec(conds.toVector))
    ))
  }
}
